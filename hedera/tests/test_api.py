...
class PersonalVocabularyQuickAddAPITest(APITestCase):
    def setUp(self):
        ...
        self.personal_vocab_list = PersonalVocabularyList.objects.create(
            user=self.created_user, lang="lat"
        )
        data = {
            "familiarity": 1,
            "headword": "testers",
        ...
        self.personal_vocab_list_entry = PersonalVocabularyListEntry.objects.create(
            **data
        )
    ...
    def test_post_personal_vocabulary_quick_add_api_with_id(self):
        ...
        response = self.client.post(
            "/api/v1/personal_vocab_list/quick_add/",
            json.dumps(payload),
    ...
    def test_successful_delete_personal_vocab_entry(self):
        payload = {"id": self.personal_vocab_list_entry.id}
        response = self.client.delete(
            "/api/v1/personal_vocab_list/",
            json.dumps(payload),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
...
class BookmarksListAPITest(APITestCase):
    def setUp(self):
        ...
        self.texts = [
            utils.create_lemmatized_text(created_by=self.user) for _ in range(3)
        ]
        self.bookmarks = [
            utils.create_bookmark(user=self.user, text=text) for text in self.texts
        ]

    def test_get_bookmarks_list(self):
        response = self.client.get(
            f"/api/v1/bookmarks/", content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class MeAPITest(APITestCase):
    ...
    def test_successful_post_me_profile_with_lang_field(self):
        response = self.client.post(
            "/api/v1/me/", json.dumps({"lang": "lat"}), content_type="application/json"
        )
    ...
    def test_failing_post_me_profile_with_lang_field(self):
        ...
        response = self.client.post(
            "/api/v1/me/", json.dumps({"lang": "abc"}), content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)
...
class BookmarksDetailAPITest(APITestCase):
    ...
    def test_get_bookmark(self):
        response = self.client.get(
            f"/api/v1/bookmarks/{self.bookmark.pk}/", content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers["Content-Type"], "application/json")
...
class SupportedLanguagesAPITest(APITestCase):
    ...
    def test_get_supported_language_list(self):
        languages = [
            [lang.code, lang.verbose_name] for lang in SUPPORTED_LANGUAGES.values()
        ]
        response = self.client.get(
            f"/api/v1/supported_languages/", content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
...
class TokenHistoryAPITest(APITestCase):
            ...
            data=[
                {
                    ...
                    "resolved": "no-lemma",
                    "following": " ",
                    "word_normalized": "Lorem",
                }
            ],
            created_by=self.is_teacher,
        )
        self.create_class = utils.create_group(
            teacher=self.is_teacher, student=self.is_student, text=self.text
        )
        self.public_text = utils.create_lemmatized_text(
            data=[
                {
                    "node": None,
                    "word": "Lorem",
    ...
    def test_successful_get_public_lemmatized_tokens(self):
        ...
        response = self.client.get(
            f"/api/v1/lemmatized_texts/{self.public_text.pk}/tokens/0/history/"
        )
        self.client.logout()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers["Content-Type"], "application/json")
    ...
    def test_successful_get_lemmatized_tokens_is_student(self):
        ...
        response = self.client.get(
            f"/api/v1/lemmatized_texts/{self.text.pk}/tokens/0/history/"
        )
        self.client.logout()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers["Content-Type"], "application/json")
