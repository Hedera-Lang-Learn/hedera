    ...
    for entry in vocab_list.entries.all():
        if entry.node is None:
            ...
            if stripped_lemma != lemma:
                l2 = get_or_create_node_for_lemma(stripped_lemma) # TODO: replace with LemmaNode.objects.filter(...)
                l2.children.add(l1)
            if morpheus and morpheus != lemma and morpheus != stripped_lemma:
                l3 = get_or_create_node_for_lemma(morpheus) # TODO: replace with LemmaNode.objects.filter(...)
                l3.children.add(l1)
